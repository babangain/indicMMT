...
model_dir=../datasets/more_noise/model_full_selattn_more_noise
...
#     --path $model_dir/checkpoint_best.pt --gen-subset test --nbest 5 \
...
fairseq-generate  $data_bin_dir \
    -s $SRC_PREFIX -t $TGT_PREFIX --task image_mmt \
    --distributed-world-size 1  --image-feat-path $image_feat_path --image-feat-dim $image_feat_dim \
    --path $model_dir/checkpoint_best.pt --gen-subset challenge --nbest 5 \
