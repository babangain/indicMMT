...
def get_cnn_features(image_list, split, batch_size, dataset_name, pretrained_cnn, pretrained_cnn_name):
    ...
    local_feats_npy = np.zeros((len(image_list),7,7,2048), dtype=np.float32)
    for start, end in zip(range(0, len(image_list)+batch_size, batch_size),
        ...
        lfeats = np.swapaxes(lfeats,3,1)
        #print(lfeats.shape)

        local_feats_npy[start:end,:,:] = lfeats.data.cpu()
    ...
    np.save(split+".en", local_feats_npy)
...
def make_dataset(args):
    ...
    if 'challenge' in args.splits:
        with open(args.challenge_fnames, 'r') as fh:
            data['challenge'] = load_fnames_into_dict(fh, 'challenge', args.images_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--challenge_fnames", type=str,
                        help="""File containing a list with Challenge image file names.""")
