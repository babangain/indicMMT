...
def get_filenames(path):
    l = []
    with open("../datasets/multimodal/"+dataset+"/"+dataset+"_files.txt", 'r') as f:
        for line in f:
            l.append(line.strip())
    return l
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, choices=['train', 'valid','test','challenge', 'test2016', 'test2017', 'testcoco'], help='which dataset')
    ...
    save_dir = os.path.join('../datasets/image_feats_full', model_name)
    ...
    with torch.no_grad():
        for i in tqdm(filenames):
            i = os.path.join(flickr30k_path, i)
