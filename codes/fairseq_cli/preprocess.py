...
def main(args):
    ...
    def make_all(lang, vocab):
        ...
        if args.challengepref:
            for k, challengepref in enumerate(args.challengepref.split(",")):
                outprefix = "challenge{}".format(k) if k > 0 else "challenge"
                _make_dataset(
                    vocab, challengepref, outprefix, lang, args=args, num_workers=args.workers
                )
    ...
    def make_all_alignments():
        ...
        if args.challengepref and os.path.exists(args.challengepref + "." + args.align_suffix):
            _make_binary_alignment_dataset(
                args.challengepref + "." + args.align_suffix,
                "challenge.align",
                num_workers=args.workers,
                args=args,
            )
