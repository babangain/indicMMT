...
image_feat_dim=768
fairseq-train ../datasets/noise/final_bin \
--max-source-positions=210 \
--max-target-positions=210 \
--max-update=20000 \
--save-interval=1 \
--arch=image_multimodal_transformer_SA_denoise_top_4x \
--task denoiser \
--criterion=label_smoothed_cross_entropy \
--source-lang=SRC \
--lr-scheduler=inverse_sqrt \
...
--adam-betas "(0.9, 0.98)" \
--clip-norm 1.0 --image-feat-path $image_feat_path --image-feat-dim $image_feat_dim --SA-image-dropout 0.3 --SA-text-dropout 0.3 --SA-attention-dropout 0.3 --is-fusion-top True \
--warmup-init-lr 1e-07 \
--warmup-updates 4000 \
--dropout 0.3 \
--tensorboard-logdir ../datasets/dataset/tensorboard-wandb \
...
--max-tokens 256 \
--lr 3e-5 \
--restore-file en-indic/model/checkpoint_best.pt --keep-interval-updates	1 --keep-best-checkpoints 1 \
--reset-lr-scheduler \
--reset-meters \
--reset-dataloader \
--reset-optimizer
