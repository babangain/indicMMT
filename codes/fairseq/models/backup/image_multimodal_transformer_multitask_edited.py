...
from fairseq.modules import (
    ...
    TransformerDecoderLayerMM,
    TransformerEncoderLayerMM,
    SelectiveAttention,
)
...
@register_model("image_multimodal_transformer_SA_multitask")
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--freeze-enc', type=bool, default=False,
                            help='Freezes encoder embedding, layers and full decoder')
        parser.add_argument('--freeze-dec', type=bool, default=False,
                            help='Freezes encoder embedding, layers and full decoder')
    @classmethod
    def build_model(cls, args, task):
        ...
        if args.freeze_enc:
            ...
            for name, param in encoder.named_parameters():
                # print(name)
                # if name == "embed_tokens.weight" or "layer" in name:
                if name == "embed_tokens.weight" or "layer" in name:
                    param.requires_grad = False
                #param.requires_grad = False
                # if name == "embed_tokens.weight":
...
class TransformerEncoder(FairseqEncoder):
    ...
    is a :class:`TransformerEncoderLayerMM`.
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        args.image_feat_dim=768

        self.args = args
        # code for image MMT

        # print(args.image_feat_dim)
        self.grid_img_fc = nn.Linear(args.image_feat_dim, embed_dim)
    ...
    def fuse_img_feat(self, text, idx, image, image_mask, text_mask):
        ...
        # res = (1 - (gate/10)) * text + (gate/10) * output
        # res = text + (gate/10) * output
        # res = (1 - (gate/10)) * text + (gate/10) * output
    ...
    def build_encoder_layer(self, args):
        return TransformerEncoderLayerMM(args)
    ...
    def forward(
        ...
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
        grid_img_features = self.grid_img_fc(imgs_list[0])
        grid_img_features = grid_img_features.transpose(0, 1)  # 49 * batch * dim
        x = torch.cat([x,grid_img_features],dim=0)
        ...
        # encoder layers
        encoder_padding_mask = torch.cat([encoder_padding_mask, grid_img_mask], dim=-1)

        # print(encoder_padding_mask.size())
        for layer in self.layers:
            x = layer(x, encoder_padding_mask,batch_len)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    is a :class:`TransformerDecoderLayerMM`.
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        return TransformerDecoderLayerMM(args, no_encoder_attn)
...
@register_model_architecture("image_multimodal_transformer_SA_multitask", "image_multimodal_transformer_SA_multitask")
def base_architecture(args):
    ...
    args.freeze_dec=getattr(args, 'freeze_dec', True)
    args.freeze_enc=getattr(args, 'freeze_enc', True)



@register_model_architecture("image_multimodal_transformer_SA_multitask", "image_multimodal_transformer_SA_multitask_top")
def image_multimodal_transformer_SA_multitask_top(args):
...
@register_model_architecture("image_multimodal_transformer_SA_multitask", "image_multimodal_transformer_SA_multitask_top_samanantar")
def image_multimodal_transformer_SA_multitask_top(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
...
@register_model_architecture("image_multimodal_transformer_SA_multitask", "image_multimodal_transformer_SA_multitask_top_4x")
def image_multimodal_transformer_SA_multitask_top(args):
