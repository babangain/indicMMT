...
def load_langpair_dataset(
    ...
    for k in itertools.count():
        ...
        denoise_dataset = data_utils.load_indexed_dataset(
            prefix + denoise, src_dict, dataset_impl
        )
        if denoise_dataset is not None:
            denoise_datasets.append(denoise_dataset)
    ...
    assert len(src_datasets) == len(tgt_datasets) == len(denoise_datasets) or len(tgt_datasets) == 0
...
@register_task("denoiser")
class DenoiserTask(LegacyFairseqTask):
    """
